...
def main(prog_args: typing.List[str]):
    parser = argparse.ArgumentParser(prog_args)
    parser.add_argument(
        "src_dir", help="The source directory of the generated codegen files"
    )
    parser.add_argument(
        "dst_dir",
        help="The destination directory to copy the generated codegen files to",
    )
    parser.add_argument(
        "--copy-codegen-files",
        help=(
    ...
    parser.add_argument(
        "--src-files-extension",
        help="The extension of the codegen sourcefiles",
        default="swift",
    )
    parser.add_argument("--debug", action="store_true", help="Turn on debugging info")
    parser.add_argument(
        "--copy-unmatched-files-dir",
        help="Directory to copy unmatched code-generated files to",
        default=None,
    )
    ...
    if args.copy_codegen_files not in {True, "true", "True"}:
        print(
            "--copy-codegen-files is not set to True: {}".format(
                args.copy_codegen_files
            )
        )
        sys.exit(0)
    ...
    if not (os.path.exists(src_dir) and os.path.isdir(src_dir)):
        print("src_dir {} does not exist!".format(src_dir), file=sys.stderr)
        sys.exit(1)

    dst_dir = os.path.abspath(args.dst_dir)
    if not (os.path.exists(dst_dir) and os.path.isdir(dst_dir)):
        print("dst_dir {} does not exist!".format(dst_dir), file=sys.stderr)
        sys.exit(1)

    if args.copy_unmatched_files_dir is not None:
        copy_unmatched_files_dir = os.path.abspath(args.copy_unmatched_files_dir)
        if not (
            os.path.exists(copy_unmatched_files_dir)
            and os.path.isdir(copy_unmatched_files_dir)
        ):
            print(
                "copy-unmatched-files-dir {} does not exist!".format(
                    copy_unmatched_files_dir
                ),
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # Get all codegen filenames and store as a dict. Apollo
    # (currently) dumps them out as a flat directory
    files_table = {
        filename.split(".")[0]: {
            "src": os.path.join(os.path.abspath(src_dir), filename),
